...
async function initializePose() {
  ...
  $('#poseStatus').textContent = '姿勢推定モデルを準備しています…';
  try {
    poseService = await createPoseService();
    $('#poseStatus').textContent = '端末内で分析中（動画は送信されません）';
    detectPose(true);
  } catch (error) {
    console.error('Pose model initialization failed', error);
    $('#poseStatus').textContent = 'モデルを読み込めませんでした';
    showToast('姿勢推定モデルの読み込みに失敗しました');
  }
}

function detectPose(force = false) {
  if (!poseService || video.readyState < 2) return;
  if (force || video.currentTime !== lastVideoTime) {
    lastVideoTime = video.currentTime;
    const result = poseService.detect(video, performance.now());
    latestLandmarks = result.landmarks?.[0];
    drawPose($('#poseCanvas'), video, latestLandmarks, $('#poseToggle').checked);
    if (video.paused) renderPoseMetrics(latestLandmarks);
  }
  if (!video.paused) detectionFrame = requestAnimationFrame(() => detectPose());
}

function renderPoseMetrics(landmarks) {
  const panel = $('#poseMetrics');
  panel.hidden = false;
  if (!landmarks) {
    $('#poseConfidence').textContent = '人物を検出できません';
    $('#metricsGrid').innerHTML = '';
    return;
  }
  const metrics = poseMetrics(landmarks);
  const labels = [['shoulder','肩の傾き'],['hip','腰の傾き'],['leftElbow','左肘の角度'],['rightElbow','右肘の角度'],['leftKnee','左膝の角度'],['rightKnee','右膝の角度']];
  $('#poseConfidence').textContent = '姿勢を検出しました';
  $('#metricsGrid').innerHTML = labels.map(([key, label]) => `<div class="metric"><span>${label}</span><strong>${metrics[key] == null ? '—' : Math.round(metrics[key])}<small>${metrics[key] == null ? '' : '°'}</small></strong></div>`).join('');
}
...
video.addEventListener('play', () => { $('#playPause').textContent = 'Ⅱ'; $('#poseMetrics').hidden = true; cancelAnimationFrame(detectionFrame); detectPose(true); });
video.addEventListener('pause', () => { $('#playPause').textContent = '▶'; cancelAnimationFrame(detectionFrame); detectPose(true); });
...
video.addEventListener('timeupdate', () => {
  ...
  if (video.paused) detectPose(true);
});
...
$('#poseToggle').addEventListener('change', () => drawPose($('#poseCanvas'), video, latestLandmarks, $('#poseToggle').checked));
window.addEventListener('resize', () => drawPose($('#poseCanvas'), video, latestLandmarks, $('#poseToggle').checked));
...
window.addEventListener('beforeunload', () => { if (objectUrl) URL.revokeObjectURL(objectUrl); poseService?.close(); });
